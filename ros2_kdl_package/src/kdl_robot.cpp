...
KDLRobot::KDLRobot(KDL::Tree &robot_tree)
{
    ...
    fdSolver_ = new KDL::ChainFdSolver_RNE(chain_,KDL::Vector(0,0,-9.81));
    jsim_.resize(n_);
    grav_.resize(n_);
    ...
    // ikAccSol_ = new KDL::ChainIkSolverAcc(chain_);
}
...
void KDLRobot::getInverseKinematicsVel(KDL::JntArray &q,KDL::Twist &t, KDL::JntArray &q_dot)
{
    int ret = ikVelSol_->CartToJnt(q,t,q_dot);
    if(ret!=0) {std::cout << ikVelSol_->strError(ret) << std::endl;};
}

// void KDLRobot::getInverseKinematicsAcc(KDl::JntArray &q, KDL::JntArray &q_dot, KDL::Twist &a, KDL::JntArray &q_ddot)
// {
//     int ret = ikAccSol_->CartToJnt(q,q_dot,a,q_ddot);
//     if(ret!=0) {std::cout << ikAccSol_->strErro(ret) << std::endl;}
// }

void KDLRobot::getForwardKinematics(KDL::Frame &f, KDL::JntArray &q, int segmentNr)
{
    int ret = fkSol_-> JntToCart(q, f, segmentNr);
...
Eigen::VectorXd KDLRobot::getFD(const KDL::JntArray &torques, 
                                const KDL::JntArray &kdl_q,
                                const KDL::JntArray &kdl_qdot)
{
    // Resize the output acceleration vector
    Eigen::VectorXd q_dotdot;
    q_dotdot.resize(chain_.getNrOfJoints());
    KDL::JntArray qdotdot(chain_.getNrOfJoints());
    KDL::Wrenches f_ext(chain_.getNrOfSegments(), KDL::Wrench::Zero());

    // Call CartToJnt with the KDL::JntArray versions of q, qdot, and torque
    int r = fdSolver_->CartToJnt(kdl_q, kdl_qdot, torques, f_ext, qdotdot);

    // Convert result to Eigen::VectorXd
    q_dotdot = qdotdot.data;
    return q_dotdot;
}
